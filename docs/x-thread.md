...
ai-skillweave changes that: one install loads **2,652 skills** + MCP servers into Claude Code, Codex, OpenClaw, Pi, Copilot, and Hermes. Structured expertise, not guesswork.
...
The batch distillation is **30-50× faster than naive per-group LLM calls** — ~580 groups processed in 2-5 minutes via batched prompts + 16 parallel workers + HTTP connection pooling, instead of 4.8 hours sequentially.
...
One command. Six harnesses. **2,652 skills**. MCP servers included. Zero config drift.
