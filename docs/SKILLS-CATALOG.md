...
**Per-harness counts (machine-specific — different users will get slightly different totals based on which sources they include and their harness versions):**

- **Claude: ~1.7K** (1,151 immediate + 550 depth-3 + 33 archive + 1 misc) — full library, native deferred loading via `/skills`
- **OpenClaw: ~1,150** (1,149 immediate + 1 misc) — workspace copies, all SKILL.md at depth-1
- **Codex: ~810** (806 from ai-skillweave + 5 Codex-bundled in `.system/`: `imagegen`, `openai-docs`, `plugin-creator`, `skill-creator`, `skill-installer`) — depth-3 flattened to depth-1
- **Pi: ~60** — curated subset of symlinks for startup performance
