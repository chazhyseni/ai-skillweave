...
**Per-harness counts (machine-specific — different users will get slightly different totals based on which sources they include and their harness versions). All counts verified with `find -L` (which follows symlinks, the way a skill loader does):**

- **Claude: 1,735** (1,151 immediate + 550 depth-2 in 63 bioSkills categories + 33 in `learned/.archive/` + 1 misc) — full library, native deferred loading via `/skills`. 0 symlinks (Claude uses real files only).
- **OpenClaw: 1,150** (1,149 immediate + 1 misc) — real file copies, 0 symlinks. All SKILL.md at depth-1.
- **Codex: 1,027 effective** = 806 real SKILL.md copies + 216 valid symlinks to source repos (ECC, Anthropic, Codex curated) + 5 Codex-bundled in `.system/` (`imagegen`, `openai-docs`, `plugin-creator`, `skill-creator`, `skill-installer`). **Plus ~127 broken symlinks** (mostly K-Dense skills pointing to the empty `~/.claude-scientific-skills/scientific-skills/` directory — a known bug).
- **Pi: 1,008 effective** = 61 curated real SKILL.md dirs (Hermes corpus hand-picks) + 947 valid symlinks to skills in source repos. **Plus ~141 broken symlinks** (same K-Dense root cause as Codex).
- **Copilot: 1,735** via symlink to `~/.claude/skills/`.

What's structural (true for every install of ai-skillweave): the pool exceeds **1,000 skills across all 5 harnesses**, served from 6 upstream repos (ECC + K-Dense + ClawBio + bioSkills + Anthropic + OpenAI Codex curated) plus Hermes's openclaw-imports staging corpus. See the README's "What Each Harness Gets" table for delivery details per harness.
