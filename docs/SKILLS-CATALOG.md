...
**Total: ~2K SKILL.md files in `~/.claude/skills/` on this system. Per-source attribution by `skill-author:` frontmatter field (sources do not double-count — Hermes openclaw-imports is a staging area, not a source):**

| Source | Count | Provenance |
|---|---|---|
| ECC | 272 | Local clone of `affaan-m/everything-claude-code` (`~/.claude-everything-claude-code/skills/`) — plus 92 commands + 67 agents (Claude-only) |
| K-Dense | 147 (107 tagged `skill-author: K-Dense Inc.`) | Cloned from `K-Dense-AI/scientific-agent-skills` (`~/.claude-scientific-skills/scientific-skills/`) |
| bioSkills | 550 (529 in local clone; 547 upstream) | Cloned from `GPTomics/bioSkills` into `~/.claude/skills/` at depth-3 (63 categories) |
| SciAgent | 197 (162 unique, 35 overlap) | Cloned from `jaechang-hits/SciAgent-Skills` (`~/.claude-sciagent-skills/skills/`) — 12 categories |
| ClawBio | 89 | Local clone of `ClawBio/ClawBio` (`~/.claude-clawbio-skills/skills/`) |
| DeepMind | 37 (all unique) | Cloned from `google-deepmind/science-skills` (`~/.claude-deepmind-skills/skills/`) |
| Bipartite | 37 | Cloned from `matsen/bipartite` (`~/.claude-bipartite/skills/`) — also installs 16 subagent definitions |
| Anthropic | 17 | Local clone in `.curated/` (`~/.claude-curated-skills/anthropic-official/skills/`) |
...
| Personal learned (current) | 0–1 (grows) | `learned/events/` — generated by `hooks/learning-capture.sh`; distilled via `extract-conversation-skills.py` |
| Personal learned (archive) | 33 | `learned/.archive/` — historical learned skills, timestamp-suffixed by harness source |
| Hermes openclaw-imports (staging) | 917 | **Not a source — staging area at `~/.hermes/skills/openclaw-imports/`.** Most overlap with ECC; not double-counted in the total. |

**Per-harness counts (machine-specific — different users get slightly different totals based on which sources they include):**

- **Claude: ~2K** — full library, native deferred loading via `/skills`. 0 symlinks (real files only).
- **OpenClaw: ~2K** — real file copies, 0 symlinks.
- **Codex: ~2K** — 806 real copies + symlinks to source repos + 5 Codex-bundled in `.system/`. 0 broken symlinks.
- **Pi: ~2K** — 61 curated real SKILL.md + symlinks to source repos. 0 broken symlinks.
- **Copilot: ~2K** via symlink to `~/.claude/skills/`.

What's structural: the pool exceeds **2,000 skills across all 5 harnesses**, served from 9 upstream repos (ECC + K-Dense + ClawBio + bioSkills + DeepMind + SciAgent + Anthropic + OpenAI Codex curated + Bipartite). See the README's "What Each Harness Gets" table for delivery details per harness.
