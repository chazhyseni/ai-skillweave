...
**Per-harness counts (machine-specific — different users will get slightly different totals based on which sources they include and their harness versions). All counts verified with `find -L` (which follows symlinks, the way a skill loader does) and a fresh `update-ecc.sh --force` run:**
...
- **OpenClaw: 1,150** (1,149 immediate + 1 misc) — real file copies, 0 symlinks. All SKILL.md at depth-1. All 107 K-Dense-authored skills are present (real copies).
- **Codex: 1,027 effective** = 806 real SKILL.md copies + 216 valid symlinks to source repos (ECC, Anthropic, Codex curated) + 5 Codex-bundled in `.system/` (`imagegen`, `openai-docs`, `plugin-creator`, `skill-creator`, `skill-installer`). 0 broken symlinks after `update-ecc.sh --force`. **However, the 107 K-Dense-authored skills are NOT visible to Codex** because `update-ecc.sh`'s `collect_skill_dirs` only scans source repos (ECC, K-Dense clone, etc.) and does NOT scan `~/.claude/skills/` itself — so skills that were imported into `~/.claude/skills/` via the Hermes openclaw-imports corpus are not in `all_skills` and never get to Codex.
- **Pi: 1,008 effective** = 61 curated real SKILL.md dirs (Hermes corpus hand-picks) + 947 valid symlinks to skills in source repos. 0 broken symlinks after `update-ecc.sh --force`. Same K-Dense gap as Codex — the 107 K-Dense skills are NOT in Pi's effective surface.
- **Copilot: 1,735** via symlink to `~/.claude/skills/`. All 107 K-Dense skills are present (inherited from Claude).

What's structural (true for every install of ai-skillweave): the pool exceeds **1,000 skills across all 5 harnesses**, served from 6 upstream repos (ECC + K-Dense + ClawBio + bioSkills + Anthropic + OpenAI Codex curated) plus Hermes's openclaw-imports staging corpus. **K-Dense skills flow only to Claude / OpenClaw / Copilot** in the current install — the gap with Codex and Pi is a known limitation of `update-ecc.sh`'s source-scanning logic (it doesn't look at `~/.claude/skills/` directly). See the README's "What Each Harness Gets" table for delivery details per harness.
