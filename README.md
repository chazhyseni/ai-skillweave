...
| `ollama launch openclaw` | **~1,210** | Real file copies (no symlinks) in `~/.openclaw/workspace/skills/`. **Note:** OpenClaw also has a `skills.entries` section in `~/.openclaw/openclaw.json` for built-in harness-native skills (e.g. obsidian, himalaya, openhue, oracle). The ai-skillweave pipeline does **not** populate `entries` — it uses the workspace dir instead, because workspace skills are loaded automatically while registered entries are opt-in. |
| `ollama launch codex` | **~1,225** (806 real SKILL.md copies + 216 valid symlinks to source repos + 5 Codex-bundled in `.system/`: `imagegen`, `openai-docs`, `plugin-creator`, `skill-creator`, `skill-installer` + 107 K-Dense symlinks from `~/.claude/skills/` via the `claude_extras` supplemental scan + 91 misc) | YAML-sanitized copies + symlinks in `~/.codex/skills/`. Codex flattens the bioSkills depth-3 layout (`<category>/<skill>/SKILL.md`) to depth-1 (`<skill>/SKILL.md`). The install creates symlinks to skills in source repos (ECC, Anthropic, Codex curated, etc.) where the skill is already in the right format, and sanitized real copies where the YAML needs repair. All 107 K-Dense-authored skills flow to Codex via the `claude_extras` supplemental scan in `update-ecc.sh` (commit `46221f7`). Codex's 64-char directory-name limit is enforced by `update-ecc.sh`; the current corpus has 0 skills over 64 chars, so no truncation actually happens. The `name:` field in SKILL.md is **never** rewritten, so cross-harness equivalence is preserved. |
| `ollama launch pi` | **~1,220** (61 curated real SKILL.md + 947 valid symlinks to skills in source repos + 107 K-Dense symlinks from `~/.claude/skills/` + 105 misc) | Symlinks in `~/.pi/agent/skills/`. Pi uses symlinks to the full corpus (unlike Codex which sometimes copies) so the effective skill surface is the union of all symlinks that resolve. The 61 curated real SKILL.md dirs are pi-specific (mostly Hermes corpus skills that have been hand-picked). The 947 symlinks point to skills in ECC + ClawBio + Anthropic + Codex curated + bioSkills (when those have SKILL.md at the symlink target). All 107 K-Dense-authored skills flow to Pi via the `claude_extras` supplemental scan in `update-ecc.sh` (commit `46221f7`); broken-symlink replacement uses `os.path.lexists()` so the new symlinks always replace stale ones. Pi also auto-discovers `~/.agents/skills/` (Agent Skills spec standard path) if you create it. |
