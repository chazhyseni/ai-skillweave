...
**~1.7K skills in Claude** (1,151 immediate-subdir + 550 in 63 bioSkills categories + 33 in `learned/.archive/` + 1 misc), **~1,150** in OpenClaw, **~1,025** in Codex, **~1,010** in Pi (Codex and Pi use symlinks to source repos, so the effective skill surface includes the 216 Codex-symlinks + 947 Pi-symlinks on top of the real files; Codex also has 5 bundled system skills in `.system/`). The Claude pool includes 107 K-Dense-authored skills (sourced via `~/.hermes/skills/openclaw-imports/`, despite the empty local `~/.claude-scientific-skills/` clone), 272 ECC skills, 89 ClawBio skills, 547 bioSkills (63 categories), 17 Anthropic, 38 unique Codex curated, plus 917-stage-corpus skills from Hermes's openclaw-imports staging area (most of which overlap with ECC). See [`docs/SKILLS-CATALOG.md`](docs/SKILLS-CATALOG.md) for the full per-source breakdown and category listing.

Numbers are machine-specific (each user gets slightly different totals based on which sources they include and what their harness versions are) — what's structural is that the pool exceeds **1,000 skills across all 5 harnesses**, served from 6 upstream repos (ECC + K-Dense + ClawBio + bioSkills + Anthropic + OpenAI Codex curated) plus Hermes's openclaw-imports staging corpus.

**Known gap:** The 107 K-Dense-authored skills are present in `~/.claude/skills/` and flow to Claude, OpenClaw, and Copilot, but they are **NOT visible to Codex or Pi** because `update-ecc.sh`'s `collect_skill_dirs` only scans source repos (ECC, K-Dense clone, etc.) and does NOT scan `~/.claude/skills/` itself. The K-Dense source clone is empty, so `update-ecc.sh` finds 0 K-Dense skills in the source and never creates symlinks/copies for Codex and Pi. The 107 K-Dense skills reach Claude via a different path (the Hermes openclaw-imports corpus already had them, and they were merged into `~/.claude/skills/` at some earlier point). A future fix to `update-ecc.sh` would extend `collect_skill_dirs` to also scan `~/.claude/skills/` for K-Dense-tagged skills, or have a dedicated sync step that re-symlinks from `~/.claude/skills/` to Codex/Pi.
...
| [K-Dense scientific](https://github.com/K-Dense-AI/scientific-agent-skills) | 0 SKILL.md dirs in the local `~/.claude-scientific-skills/` clone (empty `scientific-skills/`, no `.git`) | The repo was cloned at some point and 107 K-Dense-authored skills (tagged `skill-author: K-Dense Inc.`) **are present in `~/.claude/skills/`** — imported via `~/.hermes/skills/openclaw-imports/` (which has 108 K-Dense-tagged skills in its 917-skill corpus). The local `~/.claude-scientific-skills/` clone is now empty; the upstream repo at the time of audit also had no skill content. **The actual K-Dense footprint in the Claude pool is 107 skills, just from a different snapshot than the empty local clone suggests.** |
...
| Anthropic official | 17 SKILL.md dirs | Anthropic's official skills library (in `.curated/`) |
| OpenAI Codex curated | 44 SKILL.md dirs | OpenAI's Codex skills collection (39 in `.curated/`, 5 in `.system/`) — 38 unique names actually flow into Claude |
| **Hermes openclaw-imports corpus** (local) | 917 SKILL.md dirs | Staging area at `~/.hermes/skills/openclaw-imports/` containing skills aggregated from ECC + community + conversation-pipeline. **NOT a separate source repo** — most overlap with ECC; `origin` tag in frontmatter varies. Most skills here also exist in `~/.claude/skills/` via the same name. |
| Personal learned | varies | BMO-style real-time capture (corrections detected live via hooks) + **batched** 4-stage pipeline (30-50× faster than single-call distillation) |
...
> **Scientific skills from [K-Dense-AI/scientific-agent-skills](https://github.com/K-Dense-AI/scientific-agent-skills)** — covering bioinformatics, cheminformatics, drug discovery, clinical research, proteomics, medical imaging, ML/AI, materials science, physics, and 100+ scientific databases.

The repo is cloned and the framework is in place, but the upstream `scientific-skills/` directory is **currently empty in the snapshot we track**. When K-Dense publishes skill definitions, they will appear in this section and in `~/.claude/skills/` automatically.

To install (no-op today; activates when K-Dense publishes skills):
...
Skills are structured Markdown prompts (SKILL.md files) that tell AI agents *how to think* about specific tasks — when to activate, how to approach the problem, examples, and pitfalls. The largest single source is [GPTomics/bioSkills](https://github.com/GPTomics/bioSkills) (547 skills, 63 categories), followed by [ECC](https://github.com/affaan-m/everything-claude-code) (272 skills).
...
| K-Dense scientific | 107 skills (in `~/.claude/skills/` via Hermes openclaw-imports) | From `K-Dense-AI/scientific-agent-skills` — the local `~/.claude-scientific-skills/` clone is empty (`scientific-skills/` is empty, no `.git`), but K-Dense-authored skills are present, imported via `~/.hermes/skills/openclaw-imports/` |
...
Harnesses differ in how skills are delivered and in the per-harness count (after deduplication and codex's 64-char directory-name handling). Post-install counts on this system:

| Harness | Skill count | Delivery |
|---------|-------------|----------|
| `claude` / `ollama launch claude` | **~1.7K** (1,151 immediate-subdir + 550 in 63 bioSkills categories + 33 archived in `learned/.archive/` + 1 misc) | Native `~/.claude/skills/` — loaded on demand via `/skills`. Learned skills also injected via `lean-skills.txt` (~51 tokens) at launch. |
| `ollama launch openclaw` | **~1,150** | Real file copies (no symlinks) in `~/.openclaw/workspace/skills/`. **Note:** OpenClaw also has a `skills.entries` section in `~/.openclaw/openclaw.json` for built-in harness-native skills (e.g. obsidian, himalaya, openhue, oracle). The ai-skillweave pipeline does **not** populate `entries` — it uses the workspace dir instead, because workspace skills are loaded automatically while registered entries are opt-in. |
| `ollama launch codex` | **~1,025** (806 real SKILL.md copies + 216 valid symlinks to source repos + 5 Codex-bundled in `.system/`: `imagegen`, `openai-docs`, `plugin-creator`, `skill-creator`, `skill-installer` — 0 broken symlinks after `update-ecc.sh --force`) | YAML-sanitized copies + symlinks in `~/.codex/skills/`. Codex flattens the bioSkills depth-3 layout (`<category>/<skill>/SKILL.md`) to depth-1 (`<skill>/SKILL.md`). The install creates symlinks to skills in source repos (ECC, Anthropic, Codex curated, etc.) where the skill is already in the right format, and sanitized real copies where the YAML needs repair. **Note:** Codex does NOT see the 107 K-Dense-authored skills (which are in `~/.claude/skills/` via Hermes openclaw-imports) because `update-ecc.sh`'s `collect_skill_dirs` doesn't scan `~/.claude/skills/` directly — it only scans source repos. Codex's 64-char directory-name limit is enforced by `update-ecc.sh`; the current corpus has 0 skills over 64 chars, so no truncation actually happens. The `name:` field in SKILL.md is **never** rewritten, so cross-harness equivalence is preserved. |
| `ollama launch pi` | **~1,010** (61 curated real SKILL.md + 947 valid symlinks to skills in source repos — 0 broken symlinks after `update-ecc.sh --force`) | Symlinks in `~/.pi/agent/skills/`. Pi uses symlinks to the full corpus (unlike Codex which sometimes copies) so the effective skill surface is the union of all symlinks that resolve. The 61 curated real SKILL.md dirs are pi-specific (mostly Hermes corpus skills that have been hand-picked). The 947 symlinks point to skills in ECC + ClawBio + Anthropic + Codex curated + bioSkills (when those have SKILL.md at the symlink target). **Note:** Pi also does NOT see the 107 K-Dense-authored skills (same `update-ecc.sh` source-scan limitation as Codex). Pi also auto-discovers `~/.agents/skills/` (Agent Skills spec standard path) if you create it. |
| `copilot` (Copilot CLI) | **~1.7K** (via symlink to `~/.claude/skills`) | **Bridged** by `setup-copilot-skills.sh`: `~/.copilot/config/skills -> ~/.claude/skills` symlink + `COPILOT_SKILLS_DIRS` export pointing at Claude and Pi skill dirs. Copilot's own loader also reads `~/.claude/skills/` directly as `personal-claude`. |
...
./safe-install.sh --with-science                     # ECC + K-Dense scientific skills (currently 0 — upstream snapshot is empty)
./safe-install.sh --with-bio                         # ECC + ClawBio bioinformatics (89 skills)
./safe-install.sh --with-curated                     # Also include OpenAI Codex curated skills (currently 0 — upstream snapshot is empty)
```
...
| **lean-skills.txt** (ollama/claude CLI only) | Name + one operating principle per learned skill | ~51 tokens (currently 205 bytes; grows as you add learned skills) |
...
| Skills injection | ~1.7K files via native `/skills` + lean cache (~51 tokens) | Symlinked from `~/.claude/skills/` into Desktop sessions — same on-demand loading |
