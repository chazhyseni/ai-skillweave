...
**Note on a known bug:** `~/.codex/skills/` and `~/.pi/agent/skills/` each contain ~120-140 broken symlinks (mostly K-Dense skills pointing to the empty `~/.claude-scientific-skills/scientific-skills/` directory). These are the K-Dense skills that the install created symlinks for at a previous time when the K-Dense repo had content; now those targets don't exist, so Codex and Pi can't actually load those skills. The 107 K-Dense skills ARE present in `~/.claude/skills/` (imported via `~/.hermes/skills/openclaw-imports/`), so they show up in Claude and OpenClaw but not in Codex or Pi. A future fix to `update-ecc.sh` would either re-symlink from `~/.claude/skills/` or remove the broken symlinks.
...
| `ollama launch codex` | **~1,025** (806 real SKILL.md copies + 216 valid symlinks to source repos + 5 Codex-bundled in `.system/`: `imagegen`, `openai-docs`, `plugin-creator`, `skill-creator`, `skill-installer` — plus ~127 broken symlinks that no longer resolve) | YAML-sanitized copies + symlinks in `~/.codex/skills/`. Codex flattens the bioSkills depth-3 layout (`<category>/<skill>/SKILL.md`) to depth-1 (`<skill>/SKILL.md`). The install creates symlinks to skills in source repos (ECC, Anthropic, Codex curated, etc.) where the skill is already in the right format, and sanitized real copies where the YAML needs repair. The 127 broken symlinks are a known bug (see note above) — they are mostly K-Dense skills pointing to the empty `~/.claude-scientific-skills/scientific-skills/` directory. Codex's 64-char directory-name limit is enforced by `update-ecc.sh`; the current corpus has 0 skills over 64 chars, so no truncation actually happens. The `name:` field in SKILL.md is **never** rewritten, so cross-harness equivalence is preserved. |
| `ollama launch pi` | **~1,000** (61 curated real SKILL.md + 947 valid symlinks to skills in source repos — plus ~141 broken symlinks, mostly K-Dense) | Symlinks in `~/.pi/agent/skills/`. Pi uses symlinks to the full corpus (unlike Codex which copies) so the effective skill surface is the union of all symlinks that resolve. The 60 curated real SKILL.md dirs are pi-specific (mostly Hermes corpus skills that have been hand-picked). The 947 symlinks point to skills in ECC + ClawBio + Anthropic + Codex curated + bioSkills (when those have SKILL.md at the symlink target). The 141 broken symlinks are the K-Dense bug (same root cause as Codex). Pi also auto-discovers `~/.agents/skills/` (Agent Skills spec standard path) if you create it. |
